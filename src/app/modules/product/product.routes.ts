...
router.get("/product-details/:id", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.getProductDetails);   // Product Details
router.post("/create-products", auth(USER_ROLE.USER,USER_ROLE.OWNER), upload.array("images", 10), productController.createProduct);  // Add Product
router.put("/product-update/:id", auth(USER_ROLE.USER,USER_ROLE.OWNER), upload.array("images", 10), productController.updateProduct); // Edit Product
router.delete("/delete-product/:id", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.deleteProduct);    // Delete Product
router.post("/review/:id", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.addProductReview); // Add Review
...
router.get("/all", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.getTrending);
 
// GET /products/featured
router.get("/products/featured", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.getFeatured);
 
// GET /products/categories
router.get("/products/categories", auth(USER_ROLE.USER,USER_ROLE.OWNER), productController.getCategories);
...
router.patch(
  "/orders/:orderId",
  auth(USER_ROLE.OWNER,USER_ROLE.USER),
